...
class OakDCamera:
    def __init__(self,
        ...
        if depth == 3:
            ...
            if rgb_apply_cropping:
                # When croping to keep only smaller video
                camera.setSensorCrop(rgb_sensor_crop_x, rgb_sensor_crop_y)
                # Desired video size = ispscale result or smaller if croping
        ...
        try:
            ...
            if enable_depth:
                self.queue_xout = self.device.getOutputQueue(
                    "xout", maxSize=1, blocking=False)
                self.queue_xout_depth = self.device.getOutputQueue(
                ...
                if self.frame_xout is None:
                    raise CameraError(
                        "Unable to start OAK-D RGB and Depth camera.")

            elif enable_obstacle_dist:
                self.queue_xout = self.device.getOutputQueue(
                    "xout", maxSize=1, blocking=False)
                self.queue_xout_spatial_data = self.device.getOutputQueue(
                    "spatialData", maxSize=1, blocking=False)
    ...
    def create_depth_pipeline(self):
        ...
        stereo_manip.initialConfig.setCropRect(
    ...
    def create_obstacle_dist_pipeline(self):
        ...
        for i in range(4):
            ...
            # 30 - 40 est le mieux
            config.roi = dai.Rect(dai.Point2f(
                i*0.1+0.3, 0.35), dai.Point2f((i+1)*0.1+0.3, 0.43))
