...
class RoboHATDriver:
    ...
    def set_pulse(self, throttle, steering):
        try:
            if throttle > 0:
                output_throttle = dk.utils.map_range(throttle, 0, 1.0, 1500, 2000)
            else:
                output_throttle = dk.utils.map_range(throttle, -1, 0, 1000, 1500)

            if steering > 0:
                output_steering = dk.utils.map_range(steering, 0, 1.0, 1500, 2000)
            else:
                output_steering = dk.utils.map_range(steering, -1, 0, 1000, 1500)

            packet = "{0},{1}".format(
                str(output_throttle).zfill(4), str(output_steering).zfill(4)
            )
            self.pwm.write(b"%d, %d\r" % (eval(packet)))
            print("%d, %d\r" % (eval(packet)))
        except OSError as err:
            print(
                "Unexpected issue setting PWM (check wires to motor board): {0}".format(
                    err
                )
            )
