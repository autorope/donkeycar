...
class KerasLinear(KerasPilot):
    ...
    def __init__(
        self, num_outputs=2, input_shape=(120, 160, 3), roi_crop=(0, 0), *args, **kwargs
    ):
...
class KerasIMU(KerasPilot):
    ...
    def run(self, img_arr, accel_x, accel_y, accel_z, gyr_x, gyr_y, gyr_z):
        ...
        img_arr = img_arr.reshape((1,) + img_arr.shape)
        imu_arr = np.array([accel_x, accel_y, accel_z, gyr_x, gyr_y, gyr_z]).reshape(
            1, self.num_imu_inputs
        )
...
class KerasLocalizer(KerasPilot):
    ...
    def __init__(
        self, model=None, num_locations=8, input_shape=(120, 160, 3), *args, **kwargs
    ):
...
class KerasRNN_LSTM(KerasPilot):
    def __init__(
        ...
        **kwargs
        ...
        self.model = rnn_lstm(
            seq_length=seq_length, num_outputs=num_outputs, image_shape=image_shape
        )
...
class Keras3D_CNN(KerasPilot):
    def __init__(
        ...
        **kwargs
        ...
        self.model = build_3d_cnn(
            w=image_w, h=image_h, d=image_d, s=seq_length, num_outputs=num_outputs
        )
        self.seq_length = seq_length
        self.image_d = image_d
        self.image_w = image_w
    ...
    def compile(self):
        self.model.compile(
            loss="mean_squared_error", optimizer=self.optimizer, metrics=["accuracy"]
        )
...
class KerasLatent(KerasPilot):
    ...
    def compile(self):
        self.model.compile(
            ...
            loss_weights={"img_out": 100.0, "n_outputs0": 2.0, "n_outputs1": 1.0},
        )
