...
class SimOutput(object):
    ...
    def run(angle, throttle):
        print('sending to sim', angle, throttle)
...
class SteeringServer(object):
    ...
    def telemetry(self, sid, data):
        if data:
            ...
            self.send_control(steering * 25.0 * 2.0, throttle)
...
def serve_sim(cfg, model_path):
    ...
    ss = SteeringServer(sio, kpart=kl, image_part=img_stack)
