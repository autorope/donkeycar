...
CONFIDENCE_THRESHOLD = 0.15     # Fraction of the scan column that must match the tape colour,
                                # from 0.0 to 1.0, before the line counts as detected.
                                # The scan column is SCAN_HEIGHT pixels tall, so 0.15 means
                                # "at least 15% of the tallest matching column is tape".
                                #
                                # Measured on a real track: tape genuinely in view scored
                                # 0.28-0.44, while a ten-pixel speck at the edge of the frame
                                # that sent the car off course scored 0.125.
                                #
                                # NOTE: this used to be compared against a raw sum of mask
                                # values (255 per pixel) while being documented as a fraction,
                                # so the old default of 0.0015 could never reject anything. If
                                # you still have a value near 0.0015 the gate stays effectively
                                # open, which is the behaviour you had before; raise it to
                                # around 0.15 to actually get the protection.
