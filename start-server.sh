...
while getopts ":vbd" opt; do
  case $opt in
    ...
    d)
		...
		docker run --rm -p 8887:8887 -v ~/mydonkey:/root/mydonkey --entrypoint=/bin/bash -it --name donkey donkey
		exit
		;;
    \?)
...
docker run --rm -p 8887:8887 -v ~/mydonkey:/root/mydonkey --name donkey donkey
